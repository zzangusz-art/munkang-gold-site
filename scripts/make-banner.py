...
def soft_wall(im, w, h, sign_x=0.393, sign_y=0.51, sign_w=0.48, fill=0.56, at_x=0.5, at_y=0.47, usable=0.84):
    """간판을 칸 가운데(at_x, at_y)에 fill 비율 크기로 두고, 사진이 모자라는 곳은 사진 가장자리 톤을
    아주 흐리게 번져 채운다. 번짐 배경이 사진 가장자리와 같은 색이라 이음이 보이지 않는다.
    sign_x/sign_y: 원본에서 간판 중심, sign_w: 원본 폭 대비 간판 폭, usable: 오른쪽 기둥 전까지 쓸 폭."""
    import numpy as np
    wall = im.crop((0, 0, int(im.width * usable), im.height))
    k = w * fill / (im.width * sign_w)
    pw, ph = int(wall.width * k), int(wall.height * k)
    photo = wall.resize((pw, ph), Image.LANCZOS)
    ox = int(w * at_x - im.width * sign_x * k)
    oy = int(h * at_y - im.height * sign_y * k)
    # 배경: 축소한 사진을 가장자리 값으로 늘려(edge pad) 크게 흐린 뒤 확대
    sc = 24
    sw, sh = max(2, pw // sc), max(2, ph // sc)
    small = np.asarray(photo.resize((sw, sh), Image.BOX), dtype=np.float32)
    cw, ch = -(-w // sc), -(-h // sc)
    sx, sy = ox // sc, oy // sc
    pad = ((max(0, sy), max(0, ch - sy - sh)), (max(0, sx), max(0, cw - sx - sw)), (0, 0))
    big = np.pad(small, pad, mode='edge')
    y0, x0 = max(0, -sy), max(0, -sx)
    big = big[y0:y0 + ch, x0:x0 + cw]
    bg = Image.fromarray(big.clip(0, 255).astype('uint8')).filter(ImageFilter.GaussianBlur(2.5)).resize((cw * sc, ch * sc), Image.BICUBIC).crop((0, 0, w, h))
    # 사진 가장자리를 넓게 풀어 배경에 얹는다
    feather = int(min(pw, ph) * 0.18)
    mask = Image.new('L', (pw, ph), 0)
    ImageDraw.Draw(mask).rectangle((feather, feather, pw - feather, ph - feather), fill=255)
    mask = mask.filter(ImageFilter.GaussianBlur(feather / 2.5))
    bg.paste(photo, (ox, oy), mask)
    return grain(bg, 3)


def lighten(im, amount=1.14, warm=0.10):
    """흰 벽이 칙칙해 보이지 않게 밝히고, 사이트 배경(아이보리)과 같은 쪽으로 살짝 데운다."""
    im = ImageEnhance.Brightness(im).enhance(amount)
    return Image.blend(im, Image.new('RGB', im.size, (251, 248, 241)), warm)


def hero_desktop(src):
    """데스크톱 히어로 왼쪽 패널용 — 간판이 화면 왼쪽 절반을 채운다(글자는 오른쪽 어두운 면에 올라감)."""
    w, h = 1100, 1650  # 왼쪽 패널 비율(약 0.62~0.7)에 맞춤
    if LIGHT:
        return lighten(soft_wall(grade(src), w, h, fill=0.38, at_x=0.5, at_y=0.47))
    im = None if LIGHT else (panel(grade(src), w, h, 0.46) if DARK else cover(grade(src), w, h, focus=0.5))
    if not DARK:
...
def hero_mobile(src):
    ...
    w, h = 1200, 1400
    if LIGHT:
        return lighten(soft_wall(grade(src), w, h, fill=0.44, at_x=0.5, at_y=0.46))
    im = None if LIGHT else (panel(grade(src), w, h, 0.42) if DARK else cover(grade(src), w, h, focus=0.5))
    if not DARK:
