...
function finalize(rows) {
  // 18K·14K 매입가는 매장 정책상 순금 매입가에 비율을 곱해 낸다(18K 0.735, 14K 0.57)
  const KARAT = { au750: 0.735, au585: 0.57 };
  const pure = rows.find(r => r.code === 'au999');
  let all = rows.map(r => (pure && pure.buy != null && KARAT[r.code])
    ? { ...r, buy: pure.buy * KARAT[r.code], prev_buy: pure.prev_buy != null ? pure.prev_buy * KARAT[r.code] : null }
    : r);
