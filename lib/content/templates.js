...
<p class="cta">지금 시세로 팔거나 사시려면 <a href="/apply">매입·구매 예약</a> 또는 전화 ${esc(require('../settings').cfg('phone'))}으로 문의해 주세요. 종로3가역 1호선 2번 출구 앞 매장에서 바로 감정해 드립니다.</p>`;
...
// ── 제품 소개 초안 ──
const CAT_LABEL = { goldbar: '골드바', silverbar: '실버바', women: '순금 여성', men: '순금 남성', baby: '순금 아기', gift: '순금 기념품', jewelry: '주얼리(14K·18K)' };
...
<h2>가격은 어떻게 정해지나요?</h2>
<p>가격 = (당일 ${p.metal === 'silver' ? '은' : '금'} 판매 시세 × 순중량) + 공임(부가세 포함). ${pr.per_g ? `현재 적용 시세는 1g당 ${fmtNum(pr.per_g)}원(${esc(pr.basis)} 고시)` : '현재 시세는 시세표'}이며, 시세가 바뀌면 이 페이지의 가격도 자동으로 바뀝니다. 결제 시점의 매장 고시가가 최종 적용됩니다.</p>
<h2>구매 후 되팔 수 있나요?</h2>
<p>네. 문강금은은 자사 판매 제품을 포함한 모든 금·은 제품을 당일 매입 시세로 다시 매입합니다. 골드바·실버바는 구매 시 받은 보증서와 함께 보관해 주세요.</p>
<h2>구매 방법은?</h2>
<ul><li><strong>매장 방문</strong>: 종로3가역 1호선 2번 출구 앞. 당일 시세로 바로 구매·수령.</li><li><strong>예약 구매</strong>: <a href="/apply?kind=buy">구매 예약</a>에 제품·수량을 남기면 준비해 두었다가 방문 시 바로 드립니다.</li><li><strong>카카오톡 상담</strong>: 시세·재고·각인 문의.</li></ul>`;
  const faq = [
    ...
    { q: '부가세가 포함된 가격인가요?', a: '네. 홈페이지에 표시되는 제품 가격은 부가세가 포함된 금액입니다.' },
    { q: '되팔 때는 얼마에 매입하나요?', a: '되파는 시점의 문강금은 매입 시세(원/돈)에 순중량을 곱한 금액입니다. ' },
  ];
