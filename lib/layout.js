...
const NAV = [
  ...
  { label: '제품', href: '/products', children: [
    { label: '오늘 출발', href: '/products?category=today' }, { label: 'BEST', href: '/products?category=best' }, { label: '주얼리', href: '/products?category=jewelry' }, { label: '골드바', href: '/products?category=goldbar' }, { label: '실버바', href: '/products?category=silverbar' }, { label: '순금 기념품', href: '/products?category=gift' }] },
...
function orgLd() {
  ...
  const o = {
    ...
    url: site + '/', logo: site + '/img/logo-dark.png', image: site + '/img/og.png',
    description: `${s.site_name}은 서울 종로3가(종로3가역 11번 출구 앞)에서 금·은·백금을 매입하고 골드바·실버바·돌반지·순금 주얼리를 판매하는 금은방입니다. 매일 갱신되는 순금·18K·14K·은 시세를 공개하고, 감정 후 현장에서 현금으로 지급합니다.`,
...
<header class="header" id="header">
  ...
  <div class="wrap header-inner">
    <a class="logo" href="/" aria-label="${attr(s.site_name)} 홈"><img src="/img/logo-dark.png" alt="${attr(s.site_name)} 로고" width="170" height="64"></a>
    ${navHtml(path)}
...
<footer class="footer">
  <div class="wrap footer-grid">
    <div class="f-brand"><img src="/img/logo-dark.png" alt="" width="150" height="56" loading="lazy"><p>${esc(s.legal_name)}<br>${esc(s.slogan)}</p><p class="f-social"><a href="${attr(s.instagram)}" target="_blank" rel="noopener">Instagram</a><a href="${attr(s.youtube)}" target="_blank" rel="noopener">YouTube</a><a href="${attr(s.naver_place)}" target="_blank" rel="noopener">네이버 플레이스</a>${s.naver_blog ? `<a href="${attr(s.naver_blog)}" target="_blank" rel="noopener">네이버 블로그</a>` : ''}<a href="${attr(s.threads)}" target="_blank" rel="noopener">Threads</a>${s.inblog_url ? `<a href="${attr(s.inblog_url)}" target="_blank" rel="noopener">블로그</a>` : ''}</p>
