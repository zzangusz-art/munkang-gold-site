...
// ── 제품 가격 ──
// 순금·골드바·은: (판매가/g × 순중량 + 공임) × (1+마진) + 스톤 옵션
// 14K 주얼리: 14K 중량 ÷ 3.75 = 14K 돈 수 → × 0.6435 = 순금 환산 돈 수 → 순금 판매가(원/돈) × 순금 환산 돈 수 + 공임
// 18K 선택: 18K 중량 = 14K 중량 × 1.2 → 18K 돈 수 × 18K 계수 = 순금 환산 돈 수 (계수는 관리자 설정)
function karatFactors() {
  return { k14: settings.num('k14_pure_factor') || 0.6435, k18w: settings.num('k18_weight_factor') || 1.2, k18: settings.num('k18_pure_factor') || 0.6435 };
}
const K18 = 1.2; // 하위 호환(표기용). 실제 계산은 karatFactors().k18w
function is14k(p) { return p.quote_code === 'au585' || String(p.purity || '') === '585'; }
// 14K 중량(g) → 14K 돈 수·순금 환산 돈 수, 18K 환산까지 한 번에
function karatConv(w14) {
  const f = karatFactors(); const w = Number(w14) || 0;
  const r2 = (x) => Math.round(x * 10000) / 10000;
  const don14 = w / DON; const w18 = w * f.k18w; const don18 = w18 / DON;
  return { w14: r2(w), don14: r2(don14), pure14: r2(don14 * f.k14), w18: r2(w18), don18: r2(don18), pure18: r2(don18 * f.k18), factors: f };
}
...
function productPrice(p, opts = {}) {
  ...
  const k = is14k(p) ? karatConv(p.weight_g) : null;
  const weight = k ? (is18 ? k.w18 : k.w14) : (p.weight_g || 0);
  ...
  if (k) {
    // 14K·18K 주얼리는 순금 판매 시세로 환산해 계산
    const q = byCode('au999'); const perDon = q ? (q.sell || q.buy) : null;
    if (!perDon || !p.weight_g) return { price: null, basis: null, weight_g: weight, is18 };
    const f = k.factors; const pureDon = is18 ? (p.weight_g * f.k18w / DON) * f.k18 : (p.weight_g / DON) * f.k14; // 반올림 전 값으로 계산
    const price = Math.round((perDon * pureDon + (p.labor || 0)) * (1 + margin / 100) / 100) * 100 + add;
    return { price, basis: updatedText(), fixed: false, per_g: Math.round(perDon / DON), quote: q, weight_g: Math.round(weight * 1000) / 1000, pure_don: Math.round(pureDon * 10000) / 10000, karat_don: Math.round((is18 ? k.don18 : k.don14) * 10000) / 10000, is18, conv: true };
  }
...
module.exports = { DON, OZ, METALS, METAL_LABEL, BASE, BASE_BY_CODE, upsertRows, logUpdate, list, byCode, history, stats, gold, lastUpdated, updatedText, calc, productPrice, stoneOptions, K18, karatConv, karatFactors, is14k, spot, applySpotToQuotes, parseWorkbook, buildTemplate, withChange };
