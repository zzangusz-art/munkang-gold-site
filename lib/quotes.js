...
const BASE = [
  ...
  { code: 'au916', metal: 'gold', name: '22K', purity: '916', sort: 2, ratio: 0.916 },
  { code: 'au750', metal: 'gold', name: '18K', purity: '750', sort: 3, ratio: 0.75 },
  { code: 'au585', metal: 'gold', name: '14K', purity: '585', sort: 4, ratio: 0.585 },
...
// ── 제품 가격: (판매가/g × 순중량 + 공임) × (1+마진) ──
function productPrice(p) {
  if (p.price_fixed) return { price: p.price_fixed, basis: null, fixed: true };
  const q = p.quote_code ? byCode(p.quote_code) : null;
  const per = q ? ((q.sell || q.buy) / DON) : null; if (!per || !p.weight_g) return { price: null, basis: null };
  const margin = (p.margin_pct == null || p.margin_pct === '') ? settings.num('margin_pct') : Number(p.margin_pct);
  const price = Math.round((per * p.weight_g + (p.labor || 0)) * (1 + margin / 100) / 100) * 100;
  return { price, basis: updatedText(), fixed: false, per_g: Math.round(per), quote: q };
}
...
module.exports = { DON, OZ, METALS, METAL_LABEL, BASE, BASE_BY_CODE, upsertRows, logUpdate, list, byCode, history, stats, gold, lastUpdated, updatedText, calc, productPrice, spot, applySpotToQuotes, parseWorkbook, buildTemplate, withChange };
