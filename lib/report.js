...
<p class="muted">문의 유형: ${d.inq.byKind.map(k => `${({ sell: '매입 예약', buy: '구매', visit: '출장 매입', consult: '상담' })[k.kind] || k.kind} ${k.c}`).join(' · ') || '-'} · 인기 페이지: ${tr.topPages.slice(0, 6).map(p => `${esc(p.path)}(${p.c})`).join(' · ') || '-'}</p>
