...
router.post('/quotes/settings', (req, res) => { const b = req.body || {}; for (const k of ['quote_source', 'live_interval_min', 'live_buy_adj_pct', 'live_sell_adj_pct', 'auto_quote_from_spot', 'spot_buy_spread_pct', 'spot_sell_spread_pct', 'spot_fetch', 'margin_pct', 'labor_default', 'quote_note', 'k14_pure_factor', 'k18_weight_factor', 'k18_pure_factor']) if (k in b) setSetting(k, b[k]); res.json({ ok: true }); });

// ── 제품 ──
router.get('/karat', (req, res) => { const g = quotes.byCode('au999'); res.json({ factors: quotes.karatFactors(), pureSell: g ? (g.sell || g.buy) : null }); });
router.get('/products', (req, res) => res.json(db.prepare("SELECT id,slug,category,name,metal,purity,weight_g,quote_code,labor,margin_pct,price_fixed,badge,featured,ready_today,karat_option,stone_json,sort,status,ai_generated,updated_at,(body_html IS NOT NULL AND body_html<>'') has_body FROM products ORDER BY sort, id").all().map(p => ({ ...p, price: quotes.productPrice(p).price }))));
...
router.post('/products', (req, res) => {
  ...
  const vals = [slug, b.category, b.name, b.metal || 'gold', b.purity || '999.9', num(b.weight_g), b.quote_code || '', Number(b.labor) || 0, num(b.margin_pct), num(b.price_fixed), b.summary || '', sanitizeHtml(b.body_html || ''), b.faq_json || '[]', b.image || '', b.badge || '', b.featured ? 1 : 0, Number(b.sort) || 0, b.status || 'published', b.ready_today ? 1 : 0, b.karat_option ? 1 : 0, b.stone_json || '[]', ts];
  if (b.id) { db.prepare('UPDATE products SET slug=?,category=?,name=?,metal=?,purity=?,weight_g=?,quote_code=?,labor=?,margin_pct=?,price_fixed=?,summary=?,body_html=?,faq_json=?,image=?,badge=?,featured=?,sort=?,status=?,ready_today=?,karat_option=?,stone_json=?,updated_at=? WHERE id=?').run(...vals, b.id); return res.json({ ok: true, id: b.id }); }
  const info = db.prepare('INSERT INTO products (slug,category,name,metal,purity,weight_g,quote_code,labor,margin_pct,price_fixed,summary,body_html,faq_json,image,badge,featured,sort,status,ready_today,karat_option,stone_json,updated_at,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)').run(...vals, ts);
