...
function datasetLd(rows, updated) {
  ...
  return { '@context': 'https://schema.org', '@type': 'Dataset', name: '오늘의 금·은·백금 시세 — 문강금은 종로3가', description: `순금(24K)·22K·18K·14K·백금·은 ${rows.length}종목의 1돈(3.75g) 매입가·판매가와 전일 대비 등락. 문강금은 매장 고시가로 매일 갱신하는 1차 데이터.`, url: `${site}/price`, creator: { '@id': site + '/#org' }, license: `${site}/privacy`, dateModified: updated ? isoFromTs(updated) : kstDate(), temporalCoverage: kstDate(), spatialCoverage: '서울 종로구', keywords: ['오늘의 금시세', '금 매입가', '순금 시세', '18K 시세', '14K 시세', '은시세', '백금시세', '종로 금은방'], variableMeasured: ['매입가(원/돈)', '판매가(원/돈)', '전일 대비'], isAccessibleForFree: true, distribution: [{ '@type': 'DataDownload', encodingFormat: 'application/json', contentUrl: `${site}/api/prices` }] };
}
...
<section class="section contact-sec" id="contact"><div class="wrap contact-grid">
  ...
  <form class="inq-form reveal" id="inqForm" method="post" action="/api/inquiry" data-ajax>
    <fieldset><legend>예약 구분</legend><div class="radio-row"><label><input type="radio" name="kind" value="sell" checked> 금·은 팔기</label><label><input type="radio" name="kind" value="buy"> 골드바·제품 구매</label><label><input type="radio" name="kind" value="visit"> 출장 매입</label><label><input type="radio" name="kind" value="consult"> 상담</label></div></fieldset>
    ...
    <label>문의 내용 <textarea name="message" rows="3" maxlength="1000" placeholder="방문 희망 시간, 출장 주소, 궁금한 점"></textarea></label>
...
// ── 오늘의 금시세 ──
const METAL_META = {
  gold: { h1: '오늘의 금시세 — 순금·22K·18K·14K 매입가·판매가', intro: '순금(24K 999.9)·22K·18K·14K의 1돈(3.75g) 기준 매입가와 골드바 판매가입니다. 18K·14K는 함량(75%·58.5%)만큼 순금 대비 낮게 책정됩니다.', desc: (g, t) => `오늘의 금시세 ${t} 갱신 — 순금 24K 매입 ${g ? fmtNum(g.buy) + '원/돈' : ''}, 18K·14K·22K 매입가, 골드바 판매가, 90일 추이, 국제 금시세·환율 환산. 종로3가 문강금은 매장 고시가.`, kw: '금시세' },
...
<p>순도별 매입가 × 순중량이 기준입니다. 18K는 순금의 75%, 14K는 58.5% 함량이며 보석·부속품 무게는 제외됩니다. <a href="/calculator">매입가 계산기</a>에서 순도와 중량을 넣으면 예상 금액이 바로 나오고, 정확한 금액은 매장 감정 후 확정됩니다.</p>
...
router.get('/calculator', (req, res) => {
  ...
  const body = `
<section class="page-head"><div class="wrap"><p class="eyebrow">매입가 계산기</p><h1>금 매입가 계산기 — 순도·중량으로 예상 금액 즉시 확인</h1><p class="bluf">순금(24K)·22K·18K·14K·백금·은 중에서 순도를 고르고 중량을 g 또는 돈으로 입력하면 ${esc(st.updatedText)} 문강금은 시세 기준 예상 매입가가 계산됩니다. 순금 1돈 매입가 ${st.gold ? fmtNum(st.gold.buy) + '원' : '-'} 기준.</p></div></section>
<section class="section"><div class="wrap calc-grid">
  ...
  <aside class="side-col"><div class="side-card"><h3>단위 환산</h3><ul class="side-list"><li>1돈<span>3.75g</span></li><li>1냥(10돈)<span>37.5g</span></li><li>1트로이온스<span>31.1035g</span></li><li>반돈<span>1.875g</span></li></ul></div><div class="side-card"><h3>순도별 매입 비율</h3><ul class="side-list"><li>24K(999.9)<span>100%</span></li><li>22K(916)<span>91.6%</span></li><li>18K(750)<span>75%</span></li><li>14K(585)<span>58.5%</span></li></ul><a class="link" href="/guide/purity">순도 표기 안내 →</a></div><div class="side-card"><h3>감정 시 제외되는 것</h3><p>큐빅·다이아 등 보석, 잠금장치 스프링(철), 시계 무브먼트, 도금 제품.</p></div></aside>
</div></section>
<section class="section explain"><div class="wrap narrow"><h2>계산 공식은 무엇인가요?</h2><p><strong>예상 매입가 = 해당 순도 매입가(원/g) × 순중량(g)</strong>. 순도별 매입가는 순금 매입가에 함량 비율(18K 75%, 14K 58.5%)을 적용해 고시됩니다. 1돈은 3.75g이므로 돈 단위 입력은 3.75를 곱해 g으로 환산합니다.</p><h2>왜 실제 금액과 다를 수 있나요?</h2><ul><li>각인과 실제 순도가 다른 경우(감정 결과 적용)</li><li>보석·부속품·이물질 무게 제외</li><li>결제 시점의 고시 시세 적용</li></ul><h2>은·백금도 계산되나요?</h2><p>네. 은(Ag 999)과 백금(Pt)도 같은 방식으로 계산됩니다. Pt950·Pt900은 함량만큼 환산되므로 상담 시 안내해 드립니다.</p>${faqHtml(faqs)}</div></section>`;
...
router.get('/products', (req, res) => {
  const cat = CAT_LABEL[req.query.category] ? req.query.category : ''; const q = String(req.query.q || '').trim().slice(0, 40);
  let sql = "SELECT * FROM products WHERE status='published'"; const args = [];
  if (cat) { sql += ' AND category=?'; args.push(cat); } if (q) { sql += ' AND name LIKE ?'; args.push(`%${q}%`); }
  ...
  const body = `
<section class="page-head"><div class="wrap"><p class="eyebrow">제품</p><h1>${cat ? CAT_LABEL[cat] : '골드바·실버바·돌반지·순금 주얼리'} <small>${rows.length}개</small></h1><p class="bluf">모든 제품 가격은 ${esc(st.updatedText)} 문강금은 고시 시세(순금 판매 ${st.gold && st.gold.sell ? fmtNum(st.gold.sell) + '원/돈' : ''})에 연동되어 자동 계산되며, 결제 시점 매장 고시가가 최종 적용됩니다. 실물 귀금속 구매 시 부가세 10%가 별도이며, 구매하신 제품은 되파실 때 당일 매입 시세로 다시 매입합니다.</p>
<div class="tabs"><a class="tab${!cat ? ' active' : ''}" href="/products">전체</a>${Object.entries(CAT_LABEL).map(([k, v]) => `<a class="tab${cat === k ? ' active' : ''}" href="/products?category=${k}">${v} ${counts[k] || 0}</a>`).join('')}</div></div></section>
...
<section class="section explain"><div class="wrap narrow"><h2>제품 가격은 어떻게 정해지나요?</h2><p><strong>가격 = (당일 판매 시세 원/g × 순중량) + 공임</strong>에 소정의 마진이 더해지며, 페이지마다 적용 시세 기준시각을 표시합니다. 시세가 바뀌면 가격도 자동으로 바뀝니다.</p><h2>되팔 때는 얼마를 받나요?</h2><p>되파는 시점의 문강금은 매입 시세(원/돈) × 순중량입니다. 자사 판매 제품·타 매장 제품 모두 같은 기준이며, 보증서가 있으면 감정이 빠릅니다.</p><h2>구매는 어떻게 하나요?</h2><p>매장 방문 시 당일 시세로 바로 구매·수령하실 수 있고, <a href="/apply?kind=buy">구매 예약</a>에 제품·수량을 남기시면 준비해 두었다가 방문 시 드립니다. 온라인 결제·배송은 준비 중입니다.</p></div></section>`;
  const KWT = { goldbar: '종로 골드바 가격', silverbar: '종로 실버바 가격', baby: '종로 돌반지 가격', jewelry: '종로 금반지·금팔찌·금목걸이 가격', gift: '종로 순금 기념품 가격', coin: '종로 순금 코인 가격' };
  res.send(page({ title: `${cat ? KWT[cat] : '골드바·실버바·돌반지·순금 주얼리 가격'} — 당일 시세 연동 ${rows.length}개 제품 | 종로3가 금은방`, description: `${cat ? CAT_LABEL[cat] : '골드바·실버바·돌반지·순금 주얼리·기념품'} ${rows.length}개 제품 가격. ${st.updatedText} 시세 연동 자동 계산, 적용 기준시각 표기, 부가세 별도, 되팔 때 당일 매입. 종로3가 문강금은.`, path: '/products', body, breadcrumbs: [{ name: '제품', href: '/products' }, ...(cat ? [{ name: CAT_LABEL[cat], href: `/products?category=${cat}` }] : [])], quoteBar: quoteBar(), jsonld: [{ '@context': 'https://schema.org', '@type': 'ItemList', name: cat ? CAT_LABEL[cat] : '문강금은 제품', numberOfItems: rows.length, itemListElement: rows.slice(0, 50).map((p, i) => ({ '@type': 'ListItem', position: i + 1, name: p.name, url: `${site}/products/${encodeURIComponent(p.slug)}` })) }] }));
});
...
router.get('/products/:slug', (req, res, next) => {
  ...
  const specs = [['현재 가격', pr.price ? `<b class="big">${fmtNum(pr.price)}원</b> <small>부가세 별도</small>` : '시세 문의'], ['적용 시세', pr.basis ? `${esc(pr.basis)} 고시 · ${pr.quote ? esc(pr.quote.name) + ' 판매 ' + fmtNum(pr.quote.sell || pr.quote.buy) + '원/돈' : ''}` : (p.price_fixed ? '고정가' : '-')], ['순도', esc(p.purity || '')], ['순중량', `${p.weight_g}g (${don}돈)`], ['공임', p.labor ? fmtNum(p.labor) + '원 포함' : '없음'], ['분류', CAT_LABEL[p.category] || ''], ['되팔 때', pr.quote ? `당일 매입 시세 기준 (현재 ${fmtNum(pr.quote.buy)}원/돈 → 약 ${fmtNum(Math.round(pr.quote.buy / quotes.DON * p.weight_g))}원)` : '당일 매입 시세']];
  const body = `
...
<section class="section"><div class="wrap grid2">
  <div class="main-col">
    ...
    <div class="spec-card"><h2>${esc(p.name)} 가격·기본 정보</h2><table class="spec"><tbody>${specs.map(([k, v]) => `<tr><th>${k}</th><td>${v}</td></tr>`).join('')}</tbody></table><div class="calc-actions"><a class="btn btn-gold" href="/apply?kind=buy&item=${encodeURIComponent(p.name)}">구매 예약</a><a class="btn btn-kakao" href="${attr(s.kakao_channel)}" target="_blank" rel="noopener">카카오톡 재고 문의</a></div><p class="note">가격은 시세 연동 자동 계산값이며 결제 시점 매장 고시가가 최종 적용됩니다.</p></div></div>
