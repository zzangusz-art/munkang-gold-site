...
<p>순금(24K)·22K·18K·14K 주얼리, 골드바·코인, 돌반지, 금니(치과금), 백금(Pt) 제품, 은(실버바·은수저·은 주얼리)을 <strong>형태와 상관없이 순도·중량 기준</strong>으로 매입합니다. 끊어진 목걸이, 한 짝만 남은 귀걸이, 변색된 은도 괜찮습니다. 도금(GP·GF) 제품과 보석·부속품은 매입 대상이 아니거나 무게에서 제외됩니다.</p>
...
<ol class="steps"><li><strong>방문 또는 출장 예약</strong> — 종로3가역 11번 출구 앞 매장 방문(예약 없이 가능) 또는 <a href="/apply?kind=visit">출장 매입 예약</a>. 본인 신분증 지참.</li><li><strong>중량 측정</strong> — 고객이 보는 앞에서 전자저울로 순중량을 잽니다. 보석·부속품은 제외.</li><li><strong>순도 감정</strong> — XRF 전자 순도 측정기와 시금석으로 순도를 확인하고 결과를 함께 봅니다. 30분 이내.</li><li><strong>금액 제시</strong> — 당일 시세 × 순도 × 중량. 마음에 들지 않으면 팔지 않으셔도 됩니다.</li><li><strong>현금 지급</strong> — 동의 즉시 현장 현금 또는 계좌이체.</li></ol>
<h2 id="visit">출장 매입은 어떻게 이용하나요?</h2>
<p>수량이 많거나(예: 예물·상속 정리), 거동이 불편하거나, 매장 방문이 어려운 경우 서울·수도권으로 찾아갑니다. <a href="/apply?kind=visit">출장 매입 예약</a>에 주소·품목·대략 수량을 남기시면 일정을 잡고, 방문 담당자가 신분을 밝힌 뒤 매장과 동일한 기준으로 감정·현금 지급합니다. 출장비는 없으며, 감정 후 판매를 취소하셔도 됩니다.</p>
...
<ul><li>본인 신분증(일정 금액 이상 거래 기록용)</li><li>보증서·케이스가 있으면 감정이 빠름(없어도 무방)</li><li>여러 점을 한꺼번에 가져오시면 순도별로 나눠 감정</li><li>도금·이미테이션은 매입 불가. 헷갈리면 <a href="${attr(s.kakao_channel)}" target="_blank" rel="noopener">카카오톡</a>으로 사진 먼저 보내 주세요</li></ul>`;
  res.send(page({ title: `금·은 매입 안내 — 오늘 순금 ${g ? fmtNum(g.buy) + '원/돈' : ''} · 절차·준비물·출장 매입`, description: `종로3가 문강금은 금·은 매입 안내. 순금·18K·14K·백금·은 매입가(${st.updatedText}), 5단계 매입 절차, 준비물, 감정 방법, 출장 매입 예약. 30분 정밀 감정·현장 현금 지급.`, path: '/sell', body: shell({ eyebrow: '금 팔기', h1: '금·은 매입 안내 — 순도·중량 기준, 30분 감정, 현장 현금', bluf: `문강금은은 순금·18K·14K·백금·은 제품을 형태와 상관없이 순도·중량 기준으로 매입합니다. 오늘 순금 매입가는 ${g ? fmtNum(g.buy) + '원/돈' : '시세표 참조'}이며, 고객이 보는 앞에서 30분 이내 감정 후 현장에서 현금을 드립니다. 서울·수도권 출장 매입도 가능합니다.`, sections, faqs, side: sideQuotes() }), breadcrumbs: [{ name: '금 팔기', href: '/sell' }], jsonld: [...articleLd({ title: '금·은 매입 안내', desc: '매입 품목·절차·준비물·출장 매입', url: '/sell', faqs }), { '@context': 'https://schema.org', '@type': 'HowTo', name: '금 매입 절차', step: ['방문 또는 출장 예약', '중량 측정', '순도 감정', '금액 제시', '현금 지급'].map((n, i) => ({ '@type': 'HowToStep', position: i + 1, name: n })) }], quoteBar: quoteBar() }));
});
...
<p>문강금은은 자사 판매 골드바·실버바를 포함해 모든 금·은을 당일 매입 시세로 다시 매입합니다. 구매가와 매입가의 차이(부가세+스프레드)가 있으므로 단기 매매보다는 중장기 보유 관점을 권합니다. <a href="/guide/gold-investment">골드바 투자 안내</a>에서 자세히 설명합니다.</p>`;
  res.send(page({ title: `골드바·실버바 구매 안내 — 오늘 순금 판매 ${g && g.sell ? fmtNum(g.sell) + '원/돈' : ''} · 단위별 가격·부가세·되팔기`, description: `종로3가 문강금은 골드바·실버바 구매 안내. 1g·1돈·10g·37.5g·100g 골드바와 실버바 1kg 당일 시세 연동 가격(${st.updatedText}), 부가세 10%, 보증서, 되팔 때 매입 기준.`, path: '/buy', body: shell({ eyebrow: '금 사기', h1: '골드바·실버바 구매 안내 — 당일 시세 연동, 보증서, 재매입', bluf: `골드바·실버바 가격은 당일 판매 시세 × 순중량 + 공임으로 정해지며 부가세 10%가 별도입니다. 오늘 순금 판매 시세는 ${g && g.sell ? fmtNum(g.sell) + '원/돈' : '시세표 참조'}. 매장에서 당일 시세로 구매·수령하고, 되팔 때는 당일 매입 시세로 다시 매입합니다.`, sections, faqs, side: sideQuotes(), cta: 'buy' }), breadcrumbs: [{ name: '금 사기', href: '/buy' }], jsonld: articleLd({ title: '골드바·실버바 구매 안내', desc: '가격 구조·단위 선택·구매 방법·재매입', url: '/buy', faqs }), quoteBar: quoteBar() }));
});

// ── 가이드 ──
const GUIDES = {
  purity: { eyebrow: '금 정보', h1: '금 순도·K 표기 안내 — 24K·18K·14K, 999.9·750·585 읽는 법', bluf: '24K는 999.9(순금), 22K는 916, 18K는 750(75%), 14K는 585(58.5%)입니다. 제품 안쪽 각인으로 확인할 수 있고, GP·GF 등 도금 표기와 구분해야 합니다. 매입가는 순금 매입가에 함량 비율을 곱해 정해집니다.', title: '금 순도·K 표기 안내 — 24K·22K·18K·14K와 천분율(999.9·916·750·585)', desc: '금 순도 표기 정리: K와 천분율 대응표, 각인 위치, 도금(GP·GEP·GF) 구분, 화이트골드·핑크골드, 백금(Pt) 표기, 순도별 매입가 계산.', sections: () => `<h2>K와 천분율은 어떻게 대응되나요?</h2><table><thead><tr><th>K</th><th>천분율</th><th>순금 함량</th><th>순금 매입가 대비</th></tr></thead><tbody><tr><td>24K</td><td>999 / 999.9</td><td>99.9~99.99%</td><td>100%</td></tr><tr><td>22K</td><td>916</td><td>91.6%</td><td>약 91.6%</td></tr><tr><td>18K</td><td>750</td><td>75%</td><td>약 75%</td></tr><tr><td>14K</td><td>585</td><td>58.5%</td><td>약 58.5%</td></tr><tr><td>10K</td><td>417</td><td>41.7%</td><td>약 41.7%</td></tr></tbody></table><h2>각인은 어디를 보면 되나요?</h2><ul><li>반지: 안쪽 링 면</li><li>목걸이·팔찌: 잠금장치 또는 연결 고리 옆 작은 판</li><li>귀걸이: 침(포스트)</li><li>골드바·코인: 앞면(순도·중량·제조사)</li></ul><h2>도금은 어떻게 구분하나요?</h2><p>GP(Gold Plated)·GEP·GF(Gold Filled)·HGE 표기가 있으면 도금입니다. 자석에 붙거나 마모 부위 색이 다르면 도금 가능성이 큽니다. 확실한 방법은 매장 XRF 측정입니다.</p><h2>화이트골드·핑크골드·백금은?</h2><p>화이트골드(팔라듐·니켈 합금)와 핑크골드(구리 비율↑)는 각인 순도(750·585)대로 금 시세로 매입합니다. 백금은 Pt950·Pt900으로 표기되는 별도 금속이며 <a href="/price/platinum">백금 시세</a>로 매입합니다.</p><h2>순도별 매입가 계산</h2><p>순금 1g 매입가 × 함량 비율 × 중량. <a href="/calculator">계산기</a>에서 바로 계산됩니다.</p>`, faqs: [FAQ[1], { q: '각인이 없는 금도 매입되나요?', a: '네. XRF·시금석으로 순도를 확인해 매입합니다.' }, { q: '18K 각인인데 14K로 나올 수 있나요?', a: '드물게 각인과 실제 함량이 다를 수 있으며, 감정 결과를 고객과 함께 확인해 매입가를 정합니다.' }] },
  appraisal: { eyebrow: '금 정보', h1: '정밀 감정 안내 — 고객 참관, 30분 이내, 감정만 받아도 OK', bluf: '문강금은의 감정은 고객이 보는 앞에서 전자저울로 순중량을 재고, XRF 전자 순도 측정기와 시금석으로 순도를 확인하는 방식입니다. 대부분 30분 이내에 끝나며 감정 결과를 듣고 팔지 않으셔도 됩니다.', title: '금 정밀 감정 안내 — 저울·XRF·시금석 감정 방법과 소요 시간', desc: '금·은 감정 방법(전자저울 중량, XRF 순도 측정, 시금석), 소요 시간 30분, 고객 참관 원칙, 제외 항목(보석·부속품), 감정만 받는 경우.', sections: () => `<h2>감정은 어떤 순서로 진행되나요?</h2><ol class="steps"><li><strong>외관 확인</strong> — 각인, 보석·부속품 유무, 도금 여부 1차 확인</li><li><strong>중량 측정</strong> — 0.01g 전자저울, 보석·스프링 등 비금속 제외</li><li><strong>순도 측정</strong> — XRF(비파괴 성분 분석)로 순도 확인, 필요 시 시금석 병행</li><li><strong>결과 안내</strong> — 순도·순중량·당일 시세 × 매입가 제시</li></ol><h2>감정 방법별 차이는?</h2><table><thead><tr><th>방법</th><th>원리</th><th>특징</th></tr></thead><tbody><tr><td>XRF 측정기</td><td>X선 형광으로 성분 비율 분석</td><td>비파괴·빠름, 표면 도금 두꺼우면 보조 검사</td></tr><tr><td>시금석</td><td>돌에 긁어 시약 반응 확인</td><td>전통 방식, 미세 흠집 가능</td></tr><tr><td>비중 측정</td><td>물속 무게로 밀도 계산</td><td>골드바·코인 확인용</td></tr></tbody></table><h2>감정에서 제외되는 것은?</h2><ul><li>큐빅·다이아몬드 등 보석</li><li>잠금장치 스프링(철), 시계 무브먼트</li><li>도금·이미테이션</li></ul><h2>감정만 받아도 되나요?</h2><p>네. 감정 비용은 없으며 결과를 듣고 결정하시면 됩니다. 여러 매장을 비교하실 때는 같은 기준(1돈 vs 1g, 부속 제외 여부)으로 비교하세요.</p>`, faqs: [FAQ[3], FAQ[9], { q: '감정 비용이 있나요?', a: '없습니다. 감정만 받고 판매하지 않으셔도 됩니다.' }] },
...
router.get('/faq', (req, res) => {
  const body = `<section class="page-head"><div class="wrap"><p class="eyebrow">FAQ</p><h1>금 팔기·사기 자주 묻는 질문</h1><p class="bluf">오늘 매입가, 18K·14K 매입 비율, 준비물, 감정 방법, 출장 매입, 골드바 부가세, 돌반지 가격, 영업시간 등 실제 상담에서 가장 많이 받는 질문에 직답으로 정리했습니다.</p></div></section><section class="section"><div class="wrap narrow">${faqHtml(FAQ, '')}<p class="cta">원하는 답이 없다면 ${esc(settings.cfg('phone'))} 또는 <a href="/apply">예약·문의</a>, <a href="${attr(settings.cfg('kakao_channel'))}" target="_blank" rel="noopener">카카오톡</a>으로 물어봐 주세요.</p></div></section>`;
  res.send(page({ title: '금 매입·판매 FAQ — 오늘 매입가·18K 비율·준비물·감정·출장·부가세 질문 답변', description: '금 팔 때 준비물, 18K·14K 매입 비율, 감정 방법과 시간, 출장 매입, 골드바 부가세, 돌반지 가격, 영업시간·위치 등 종로3가 문강금은 자주 묻는 질문.', path: '/faq', body, breadcrumbs: [{ name: '자주 묻는 질문', href: '/faq' }], jsonld: [faqLd(FAQ)], quoteBar: quoteBar() }));
});
...
router.get('/apply', (req, res) => {
  ...
  const body = `<section class="page-head"><div class="wrap"><p class="eyebrow">예약·문의</p><h1>매입·구매·출장 예약</h1><p class="bluf">품목과 대략적인 중량(팔 때) 또는 제품·수량(살 때)을 남겨 주시면 영업시간(${esc(s.hours_open)}–${esc(s.hours_close)}) 내 바로 연락드려 시세와 일정을 안내합니다. 급하시면 ${esc(s.phone)}으로 전화 주세요. 감정만 받아도 되고, 예약 없이 매장 방문도 가능합니다.</p></div></section>
<section class="section"><div class="wrap grid2">
  <form class="inq-form big reveal" method="post" action="/api/inquiry" data-ajax>
    <fieldset><legend>예약 구분</legend><div class="radio-row">${[['sell', '금·은 팔기'], ['buy', '골드바·제품 구매'], ['visit', '출장 매입'], ['consult', '상담(리세팅·투자)']].map(([k, v]) => `<label><input type="radio" name="kind" value="${k}"${kind === k ? ' checked' : ''}> ${v}</label>`).join('')}</div></fieldset>
    ...
    <label>문의 내용 <textarea name="message" rows="5" maxlength="1500" placeholder="방문 희망 일시, 출장 주소(출장 매입 시), 각인 문구, 궁금한 점"></textarea></label>
  ...
  <aside class="side-col"><div class="side-card"><h3>접수 후 절차</h3><ol class="side-steps"><li>영업시간 내 담당자 연락</li><li>시세·방문(출장) 일정 안내</li><li>매장 감정 → 현금 지급 / 제품 수령</li></ol></div><div class="side-card"><h3>바로 연락</h3><a class="btn btn-gold block" href="tel:${attr(s.phone)}">${esc(s.phone)}</a><a class="btn btn-kakao block" href="${attr(s.kakao_channel)}" target="_blank" rel="noopener">카카오톡 상담</a><p class="note">${esc(s.address)} · ${esc(s.hours)}</p></div>${sideQuotes()}</aside>
</div></section>`;
  res.send(page({ title: '매입·구매·출장 예약 — 품목·중량만 남기면 영업시간 내 연락', description: `종로3가 문강금은 금·은 매입 예약, 골드바·돌반지 구매 예약, 출장 매입 신청, 리세팅·투자 상담. 전화 ${s.phone}, 카카오톡 상담, ${s.hours}.`, path: '/apply', body, breadcrumbs: [{ name: '예약·문의', href: '/apply' }], quoteBar: quoteBar() }));
});
...
router.get('/reviews', (req, res) => {
  ...
  const body = `<section class="page-head"><div class="wrap"><p class="eyebrow">고객 후기</p><h1>실제 거래 고객 후기 <small>${rows.length}건${rows.length ? ` · 평균 ${avg}점` : ''}</small></h1><p class="bluf">매장·출장 매입, 골드바·돌반지 구매 고객이 남긴 후기입니다. 게재에 동의한 후기만 표시하며, 당근 업체 프로필의 후기는 <a href="${attr(s.daangn)}" target="_blank" rel="noopener">여기</a>에서 볼 수 있습니다.</p></div></section><section class="section"><div class="wrap">${rows.length ? `<div class="rv-grid">${rows.map(r => `<blockquote class="rv reveal"><span class="stars">${'★'.repeat(r.rating)}</span><p>${esc(r.text)}</p><footer>${esc(r.name)} · ${esc(r.kind || '')} · ${kstDate(new Date(r.created_at * 1000))}</footer></blockquote>`).join('')}</div>` : `<p class="note">후기를 준비 중입니다. 거래 후 후기를 남겨 주시면 감사드립니다.</p>`}</div></section>`;
...
<h2>무엇을 하나요?</h2><table><thead><tr><th>서비스</th><th>내용</th></tr></thead><tbody><tr><td>금·은 매입</td><td>순금·22K·18K·14K·백금·은 제품, 골드바·코인, 돌반지, 금니 — 30분 정밀 감정 후 현장 현금</td></tr><tr><td>골드바·실버바 판매</td><td>1g~100g 골드바, 실버바 100g~1kg, 보증서 제공, 당일 시세 재매입</td></tr><tr><td>돌반지·순금 선물</td><td>돌반지·돌팔찌·금수저·행운의 열쇠·12지신 오브제, 각인·케이스</td></tr><tr><td>출장 매입</td><td>서울·수도권, 예약제, 매장과 동일 기준</td></tr><tr><td>리세팅·상담</td><td>헌 금으로 새 주얼리, 골드바 적립 상담</td></tr></tbody></table>
<h2>왜 문강금은을 선택하나요?</h2><ul><li><strong>시세 공개</strong>: 매입가·판매가를 돈·g 단위로 매일 공개(${esc(st.updatedText)} 갱신). 다른 매장과 비교하기 쉽습니다.</li><li><strong>참관 감정</strong>: 저울과 측정기를 고객 쪽으로 돌려 함께 확인합니다.</li><li><strong>현장 현금</strong>: 동의 즉시 현금 또는 계좌이체.</li><li><strong>연중무휴</strong>: 매일 ${esc(s.hours_open)}–${esc(s.hours_close)}, 전화 상담 24시간.</li></ul>
...
<section class="section"><div class="wrap grid2"><div class="main-col"><div class="map-box"><iframe title="문강금은 위치 지도" src="https://www.google.com/maps?q=${encodeURIComponent('종로3가역 11번 출구')}&output=embed&hl=ko" width="100%" height="400" loading="lazy" referrerpolicy="no-referrer-when-downgrade" style="border:0;border-radius:14px"></iframe></div><article class="prose"><h2>어떻게 오나요?</h2><ul><li><strong>지하철</strong>: 1·3·5호선 종로3가역 <strong>11번 출구</strong> 바로 앞.</li><li><strong>버스</strong>: 종로3가 정류장 하차 후 도보 2~3분.</li><li><strong>주차</strong>: 인근 공영·민영 주차장 이용(출장 매입 시 방문 필요 없음).</li></ul><h2>방문 전 준비</h2><p>팔려는 금·은 제품과 신분증만 가져오시면 됩니다. 구매는 제품·수량을 <a href="/apply?kind=buy">예약</a>해 두시면 대기 없이 수령하실 수 있습니다.</p></article></div><aside class="side-col"><div class="side-card"><h3>연락처</h3><p>${esc(s.legal_name)}<br>${esc(s.address)}<br>전화 <a href="tel:${attr(s.phone)}">${esc(s.phone)}</a>${s.phone2 ? `<br>전화 <a href="tel:${attr(s.phone2)}">${esc(s.phone2)}</a>` : ''}<br>${esc(s.hours)}</p><a class="btn btn-gold block" href="https://map.naver.com/p/search/${encodeURIComponent('문강금은 종로3가')}" target="_blank" rel="noopener">네이버 지도에서 보기</a><a class="btn btn-kakao block" href="${attr(s.kakao_channel)}" target="_blank" rel="noopener">카카오톡 상담</a></div></aside></div></section>`;
...
<h2>1. 수집하는 개인정보 항목과 방법</h2><p>매입·구매·출장 예약 및 상담 신청 시 성함, 연락처(휴대전화), 이메일(선택), 품목·중량·문의 내용을 수집합니다. 서비스 이용 과정에서 접속 IP, 브라우저 정보, 유입 경로가 자동 수집될 수 있습니다.</p>
<h2>2. 수집·이용 목적</h2><ul><li>금·은 매입·판매·출장·리세팅 상담 및 예약 진행</li><li>상담 결과 안내 및 문의 응대</li><li>서비스 개선을 위한 통계(개인 식별 불가 형태)</li></ul>
...
const KW = {
  ...
  '종로금은방': { h: '종로 금은방 문강금은 — 신뢰로 거래하는 종로3가 금은방', a: '종로 금은방 문강금은은 종로3가역 11번 출구 앞에서 금·은 매입, 골드바·돌반지·순금 주얼리 판매, 출장 매입을 운영합니다.', to: '/about', cat: 'jewelry' },
