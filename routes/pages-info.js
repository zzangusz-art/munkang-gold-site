...
<ol class="steps"><li><strong>매장 방문</strong> — 종로3가역 1호선 2번 출구 앞 매장으로 오세요(예약 없이 가능). 본인 신분증 지참.</li><li><strong>중량 측정</strong> — 고객이 보는 앞에서 전자저울로 순중량을 잽니다. 보석·부속품은 제외.</li><li><strong>순도 감정</strong> — 시금석과 시약으로 순도를 확인하고 결과를 함께 봅니다. 30분 이내에 끝납니다.</li><li><strong>금액 제시</strong> — 당일 시세 × 순도 × 중량. 마음에 들지 않으면 팔지 않으셔도 됩니다.</li><li><strong>현금 지급</strong> — 동의 즉시 현장에서 현금으로 드립니다(계좌이체는 하지 않습니다).</li></ol>
...
<h2>준비물과 주의사항</h2>
<ul><li>본인 신분증(일정 금액 이상 거래 기록용)</li><li>여러 점을 한꺼번에 가져오시면 순도별로 나눠 감정</li><li>도금·이미테이션은 매입 불가. 헷갈리면 <a href="${attr(s.kakao_channel)}" target="_blank" rel="noopener">카카오톡</a>으로 사진 먼저 보내 주세요</li></ul>`;
...
<p><strong>가격 = (당일 판매 시세 원/g × 순중량) + 공임</strong>이며, 홈페이지에 표시되는 가격은 <strong>부가세가 포함된 금액</strong>입니다. 오늘 순금 판매 시세는 ${g && g.sell ? fmtNum(g.sell) + '원/돈(' + fmtNum(g.sellG) + '원/g)' : '시세표 참조'}${sv && sv.sell ? `, 은 판매 시세는 ${fmtNum(sv.sell)}원/돈` : ''}입니다. 제품 페이지의 가격은 이 시세에 자동 연동되며 적용 기준시각을 표시합니다.</p>
<table><thead><tr><th>제품</th><th>순도</th><th>순중량</th><th>오늘 가격(부가세 포함)</th></tr></thead><tbody>${goldbars.map(p => { const pr = quotes.productPrice(p); return `<tr><td><a href="/products/${attr(p.slug)}">${esc(p.name)}</a></td><td>${esc(p.purity)}</td><td>${p.weight_g}g</td><td>${pr.price ? fmtNum(pr.price) + '원' : '문의'}</td></tr>`; }).join('')}</tbody></table>
...
<h2>구매 방법은?</h2>
<ol class="steps"><li><strong>매장 방문</strong> — 종로3가역 1호선 2번 출구 앞. 당일 시세로 바로 구매·수령, 보증서 제공.</li><li><strong>구매 예약</strong> — <a href="/apply?kind=buy">예약 페이지</a>에 제품·수량을 남기면 준비해 두었다가 방문 시 바로 드립니다.</li><li><strong>카카오톡 상담</strong> — 재고·각인·선물 포장 문의.</li></ol>
...
const GUIDES = {
  ...
  appraisal: { eyebrow: '금 정보', h1: '정밀 감정 안내 — 고객 참관, 30분 이내, 감정만 받아도 OK', bluf: '문강금은의 감정은 고객이 보는 앞에서 전자저울로 순중량을 재고, 시금석과 시약으로 순도를 확인하는 방식입니다. 대부분 30분 이내에 끝나며 감정 결과를 듣고 팔지 않으셔도 됩니다.', title: '금 정밀 감정 안내 — 저울·시금석·시약 감정 방법과 소요 시간', desc: '금·은 감정 방법(전자저울 중량, 시금석·시약 순도 확인), 소요 시간 30분, 고객 참관 원칙, 제외 항목(보석·부속품), 감정만 받는 경우.', sections: () => `<h2>감정은 어떤 순서로 진행되나요?</h2><ol class="steps"><li><strong>외관 확인</strong> — 각인, 보석·부속품 유무, 도금 여부 1차 확인</li><li><strong>중량 측정</strong> — 0.01g 전자저울, 보석·스프링 등 비금속 제외</li><li><strong>순도 측정</strong> — 시금석에 긁어 시약 반응으로 순도 확인, 필요 시 비중 측정 병행</li><li><strong>결과 안내</strong> — 순도·순중량·당일 시세 × 매입가 제시</li></ol><h2>감정 방법별 차이는?</h2><table><thead><tr><th>방법</th><th>원리</th><th>특징</th></tr></thead><tbody><tr><td>시금석·시약</td><td>돌에 긁어 시약 반응 확인</td><td>전통 방식, 미세 흠집 가능</td></tr><tr><td>비중 측정</td><td>물속 무게로 밀도 계산</td><td>골드바·코인 확인용</td></tr></tbody></table><h2>감정에서 제외되는 것은?</h2><ul><li>큐빅·다이아몬드 등 보석</li><li>잠금장치 스프링(철), 시계 무브먼트</li><li>도금·이미테이션</li></ul><h2>감정만 받아도 되나요?</h2><p>네. 감정 비용은 없으며 결과를 듣고 결정하시면 됩니다. 여러 매장을 비교하실 때는 같은 기준(1돈 vs 1g, 부속 제외 여부)으로 비교하세요.</p>`, faqs: [FAQ[3], FAQ[9], { q: '감정 비용이 있나요?', a: '없습니다. 감정만 받고 판매하지 않으셔도 됩니다.' }] },
...
router.get('/about', (req, res) => {
  ...
  const body = `<section class="page-head"><div class="wrap"><p class="eyebrow">매장 소개</p><h1>${esc(s.site_name)} — 종로3가 금·은 매입·판매 전문</h1><p class="bluf">${esc(s.site_name)}은 서울 종로3가 귀금속 거리, 종로3가역 1호선 2번 출구 바로 앞에 있는 금은방입니다. "신뢰"를 원칙으로 매입가·판매가를 매일 공개하고, 고객이 보는 앞에서 감정한 뒤 현장에서 현금으로 드립니다. 골드바·실버바·주얼리·돌반지 판매, 주얼리 리세팅, 금 투자 상담을 제공합니다.</p></div></section>
<section class="section"><div class="wrap grid2"><div class="main-col">
<figure class="store-photo reveal"><img src="/img/store.jpg" alt="문강금은 매장 내부 — 벽면 간판 MUNKANG GOLD EXCHANGE" width="1400" height="800" loading="lazy"><figcaption>종로3가역 1호선 2번 출구 앞 문강금은 매장</figcaption></figure>
<div class="greeting reveal"><p>금과 은을 다루는 일에는 무엇보다 <b>'신뢰'</b>가 우선이라고 생각합니다.</p><p>문강금은은 그 믿음 하나로 오랜 시간 고객님들과 함께해왔습니다. 화려함보다 진심을, 겉치레보다 정직한 거래를 우선으로 하는 것이 저희의 원칙입니다. 앞으로도 투명하고 믿을 수 있는 문강금은으로 곁에 있겠습니다.</p><div class="badges"><span>검증된 제품 퀄리티</span><span>종로의 압도적으로 합리적인 가격</span><span>문의전화 ${esc(s.phone2)}</span></div><p>취급 품목은 금, 은이며, 정밀 감정을 거쳐 30분 이내로 매입 및 현장 현금 지급이 가능합니다. 이외에도 주얼리와 돌반지 판매, 골드바 매매까지 폭넓게 운영하고 있습니다.</p><p><b>주요 서비스는 금·은 판매·매입과 투자 상담입니다.</b> 국제 시세는 시시각각 변합니다. 그 흐름에 맞춰 골드바 투자 상담을 진행해드리며, 감정 후 30분 이내 매입, 즉시 현금 지급까지 한 번에 처리해드립니다.</p><p>금과 은, 그 하나하나에 담긴 이야기를 소중히 다루는 곳, 문강금은입니다.</p><p>짧은 기간이지만 동안 걸어온 길, 앞으로도 정직함을 바탕으로 고객님과의 신뢰를 이어가겠습니다. 감사합니다.</p><p class="sig">문강금은 드림 · 종로3가역 1호선 2번 출구 앞</p></div>
<article class="prose">
...
<h2>매장 정보</h2><table class="spec"><tbody><tr><th>상호</th><td>${esc(s.legal_name)} (${esc(s.en_name)})</td></tr>${s.ceo ? `<tr><th>대표</th><td>${esc(s.ceo)}</td></tr>` : ''}${s.biz_no ? `<tr><th>사업자등록번호</th><td>${esc(s.biz_no)}</td></tr>` : ''}<tr><th>주소</th><td>${esc(s.address)}${s.address_detail ? ' ' + esc(s.address_detail) : ''}</td></tr><tr><th>전화</th><td>${esc(s.phone)}${s.phone2 ? ' / ' + esc(s.phone2) : ''}</td></tr><tr><th>영업시간</th><td>${esc(s.hours)}</td></tr><tr><th>업종</th><td>귀금속 소매·매입(금은방)</td></tr><tr><th>상담 채널</th><td><a href="${attr(s.kakao_channel)}" target="_blank" rel="noopener">카카오 오픈채팅</a> · 전화 ${esc(s.phone)} · 카카오톡 ID ${esc(s.kakao_id)}</td></tr><tr><th>공식 채널</th><td><a href="${attr(s.naver_place)}" target="_blank" rel="noopener">네이버 플레이스</a> · <a href="${attr(s.naver_blog)}" target="_blank" rel="noopener">네이버 블로그</a> · <a href="${attr(s.instagram)}" target="_blank" rel="noopener">Instagram</a> · <a href="${attr(s.youtube)}" target="_blank" rel="noopener">YouTube</a> · <a href="${attr(s.threads)}" target="_blank" rel="noopener">Threads</a> · <a href="${attr(s.daangn)}" target="_blank" rel="noopener">당근</a></td></tr></tbody></table>
  ...
  res.send(page({ title: `매장 소개 — ${s.site_name} (종로3가역 1호선 2번 출구 앞 금·은 매입·판매 금은방)`, description: `${s.site_name}은 종로3가 귀금속 거리의 금은방으로 매입가·판매가를 매일 공개하고 고객 참관 감정, 현장 현금 지급, 골드바·주얼리·돌반지 판매를 제공합니다. ${s.hours}.`, path: '/about', body, breadcrumbs: [{ name: '문강금은', href: '/about' }], jsonld: [{ '@context': 'https://schema.org', '@type': 'AboutPage', name: '매장 소개', url: site() + '/about', mainEntity: { '@id': site() + '/#org' } }], quoteBar: quoteBar() }));
});
router.get('/about/location', (req, res) => {
  const s = settings.all();
  const body = `<section class="page-head"><div class="wrap"><p class="eyebrow">문강금은</p><h1>찾아오시는 길 — 종로3가역 1호선 2번 출구 앞</h1><p class="bluf">${esc(s.address)}${s.address_detail ? ' ' + esc(s.address_detail) : ''}. 지하철 1·3·5호선 종로3가역 1호선 2번 출구로 나오면 바로 앞입니다. ${esc(s.hours)}. 방문 전 전화(${esc(s.phone)})나 카카오톡으로 당일 시세를 확인하실 수 있습니다.</p></div></section>
<section class="section"><div class="wrap grid2"><div class="main-col"><div class="map-box"><a class="map-card lg" href="${attr(s.naver_place)}" target="_blank" rel="noopener"><b>네이버 지도로 길찾기</b><span>${esc(s.address)}</span><span class="map-go">네이버 지도 열기 →</span></a></div><article class="prose"><h2>어떻게 오나요?</h2><ul><li><strong>지하철</strong>: 1·3·5호선 종로3가역 <strong>1호선 2번 출구</strong> 바로 앞.</li><li><strong>버스</strong>: 종로3가 정류장 하차 후 도보 2~3분.</li><li><strong>주차</strong>: 인근 공영·민영 주차장을 이용해 주세요.</li></ul><h2>방문 전 준비</h2><p>팔려는 금·은 제품과 신분증만 가져오시면 됩니다. 구매는 제품·수량을 <a href="/apply?kind=buy">예약</a>해 두시면 대기 없이 수령하실 수 있습니다.</p></article></div><aside class="side-col"><div class="side-card"><h3>연락처</h3><p>${esc(s.legal_name)}<br>${esc(s.address)}<br>전화 <a href="tel:${attr(s.phone)}">${esc(s.phone)}</a>${s.phone2 ? `<br>전화 <a href="tel:${attr(s.phone2)}">${esc(s.phone2)}</a>` : ''}<br>${esc(s.hours)}</p><a class="btn btn-gold block" href="https://map.naver.com/p/search/${encodeURIComponent('문강금은 종로3가')}" target="_blank" rel="noopener">네이버 지도에서 보기</a><a class="btn btn-kakao block" href="${attr(s.kakao_channel)}" target="_blank" rel="noopener">카카오톡 상담</a></div></aside></div></section>`;
  res.send(page({ title: '찾아오시는 길 — 종로3가역 1호선 2번 출구 앞, 매일 10~20시', description: `문강금은 오시는 길. ${s.address}, 지하철 1·3·5호선 종로3가역 1호선 2번 출구 바로 앞. ${s.hours}. 전화 ${s.phone}.`, path: '/about/location', body, breadcrumbs: [{ name: '문강금은', href: '/about' }, { name: '찾아오시는 길', href: '/about/location' }] }));
});
...
const KW = {
  '종로금거래소': { h: '종로 금거래소 문강금은 — 금·은 매입·판매, 오늘 시세 공개', a: '종로3가역 1호선 2번 출구 앞 문강금은은 금·은을 매입·판매하는 종로 금거래소(금은방)입니다. 순금·18K·14K·은 시세를 매일 공개하고 30분 이내 감정 후 현장 현금을 드립니다.', to: '/about', cat: 'goldbar' },
  '종로3가금거래소': { h: '종로3가 금거래소 — 종로3가역 1호선 2번 출구 앞 문강금은', a: '종로3가역 1호선 2번 출구 바로 앞 문강금은은 금·은 매입과 골드바·돌반지 판매를 함께 하는 종로3가 금거래소입니다. 매일 10~20시 연중무휴.', to: '/about/location', cat: 'goldbar' },
  ...
  '종로3가금매입': { h: '종로3가 금매입 — 역 1호선 2번 출구 앞, 감정 참관·현금 지급', a: '종로3가에서 금을 파실 때는 매입가 공개 여부, 1돈 기준인지, 부속품 제외 여부를 확인하세요. 문강금은은 세 가지를 모두 공개합니다.', to: '/sell', cat: '' },
  ...
  '종로금은방': { h: '종로 금은방 문강금은 — 신뢰로 거래하는 종로3가 금은방', a: '종로 금은방 문강금은은 종로3가역 1호선 2번 출구 앞에서 금·은 매입과 골드바·주얼리·돌반지 판매를 운영합니다.', to: '/about', cat: 'jewelry' },
  '종로3가금은방': { h: '종로3가 금은방 — 종로3가역 1호선 2번 출구 앞 문강금은', a: '종로3가 금은방을 찾으신다면 지하철 1호선 2번 출구 바로 앞 문강금은으로 오세요. 매일 10~20시, 전화 상담 24시간.', to: '/about/location', cat: 'jewelry' },
...
<h2>${esc(kw)}, 무엇을 확인해야 하나요?</h2><ul><li><strong>시세 공개 여부</strong>: 매입가·판매가를 돈·g 단위로 공개하는지. 문강금은은 <a href="/price">시세표</a>를 매일 갱신합니다.</li><li><strong>감정 방식</strong>: 고객이 보는 앞에서 저울과 시금석으로 확인하는지. <a href="/guide/appraisal">감정 안내</a></li><li><strong>현금 지급·재매입</strong>: 매입은 현장 현금, 판매 제품은 되팔 때 당일 매입 시세 적용.</li><li><strong>위치·시간</strong>: 종로3가역 1호선 2번 출구 앞, ${esc(s.hours)}.</li></ul>
